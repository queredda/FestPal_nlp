...
class TestChatBotCore:
    ...
    def test_identity_questions(self, bot_instance):
        """Test 2: Bot correctly identifies itself"""
        identity_questions = [
            "siapa namamu?",
            "who are you",
            "siapa kamu",
    ...
    def test_refund_processing(self, bot_instance):
        ...
        assert "456" in response or "ABC456" in response  # Either original or reflected
    ...
    def test_help_functionality(self, bot_instance):
        ...
        for question in help_questions:
            ...
            assert "•" in response  # Bullet points for features
    ...
    def test_resale_transfer_warnings(self, bot_instance):
        ...
        for question in resale_questions:
            response = bot_instance.reply(question)
            assert "⚠️" in response or "berisiko" in response.lower()
    ...
    def test_how_to_buy_tickets(self, bot_instance):
        ...
        # Test questions that match the "cara beli" pattern
        cara_questions = [
            "cara beli tiket",
            "cara membeli tiket"
        ]
        ...
        # Test English questions that may not match exact pattern but should get some response
        english_questions = [
        ...
        for question in english_questions:
            response = bot_instance.reply(question)
            # Should get some helpful response, not just default fallback
            assert len(response) > 0
    ...
    def test_generic_fallback_keywords(self, bot_instance):
        ...
        # Test "aturan" separately as it has its own specific response
...
class TestUtilityFunctions:
    ...
    def test_reflect_function(self):
        """Test reflection mapping works correctly"""
        test_cases = [
    ...
    def test_festival_info_structure(self):
        ...
        # Check parking info completeness
        ...
        for day_lineup in lineup.values():
            ...
            for artist_info in day_lineup:
                assert len(artist_info) == 2  # (artist, time) tuple
